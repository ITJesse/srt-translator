...
export const systemPrompt = (
  ...
  const glossarySection =
    glossary && Object.keys(glossary).length > 0
      ? `
【术语表是翻译过程中最重要的参考资源，必须严格遵循】
专有名词对照表，如果原文中出现了下列词语，直接使用对应的翻译替换（请严格按照此表翻译以下专有名词，不允许有任何偏差）：
...
${glossarySection}
遵循以下翻译规则：
1. 【最高优先级】严格按照专有名词对照表翻译相关术语，这是确保翻译一致性和专业性的关键
2. 充分理解${sourceLanguage}的语言特点和文化背景
3. 翻译成自然、地道的${targetLanguage}，避免生硬的直译
4. 保持专业术语的准确性和一致性
5. 考虑${targetLanguage}的文化语境，适当本地化内容
6. 确保译文符合${targetLanguage}的语法和表达习惯

需要无比严格地遵循以下规则：
1. 必须严格保证每个哈希值的输入都有对应的翻译
2. 不允许省略任何给定哈希值输入的翻译
3. 术语表中的每个术语必须按照指定的翻译使用，不得有任何变化
...
【注意】：上例中"Star Trek"必须翻译为"星际迷航"，"Spock"必须翻译为"斯波克"，不允许使用其他翻译方式。
